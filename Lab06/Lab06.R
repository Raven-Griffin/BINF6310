...
# Load and normalize counts table
...
# Define group categories and numeric time (in days)
# Cols 1-3: Day 2 | Cols 4-6: Week 12 (~86 days) | Cols 7-11: Week 20 (~128 days)
...
# Loop over each gene and fit models
for (i in 1:nrow(myTNorm)) {
  index[i] <- i
  myData    <- as.numeric(myTNorm[i, ])
  
  #One-way ANOVA with three time-point categories
  lmA <- lm(myData ~ cats)
  pValuesOneWayAnova[i] <- anova(lmA)$"Pr(>F)"[1]
  
  # Linear regression over continuous time in days
  lmB <- lm(myData ~ days)
  pValuesRegression[i] <- anova(lmB)$"Pr(>F)"[1]
  
  # F-test comparing model A (3 params) vs model B (2 params)
...
# Build data frame and apply BH FDR correction
...
# Sort frames to find top gene for each model
...
# Top gene plot for model A: boxplot by category
...
# Top gene plot for model B: scatter plot with regression line
...
# Top gene plot for model C: boxplot by category
